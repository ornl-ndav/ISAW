...
public class Method2OperatorWizard extends JFrame implements ActionListener {
     ...
     *         strings included.
     */
    public static String FixUpMultiLines(String S, String LinePrefix, 
                                            String LinePostFix) {
        ...
        if (k1 < 0) k1 = S.length();
        while (k < S.length()) {
            Res += LinePrefix + S.substring(k, k1).trim() + LinePostFix;
            k = k1 + 1;
            if (k1 < S.length())
                k1 = S.indexOf('\n', k1 + 1);
        }
            ...
            if (evt.getActionCommand().equals("FileName:Method")) {
                ...
                for (int k = 0; (k < pinf.getNParamTypes()) && !f; k++)
                    if (pinf.getType(k).equals(T)) {
                        ...
                        jf.setSize(300, 300);
                        jf.getContentPane().add(new JEditorPane("text/html", pinf.getToolTip(k, false)));
        ...
        public void SetMethodList(String fileName) {
            ...
            try {
                ...
                MethodList.setBorder(BorderFactory.createTitledBorder( 
                        BorderFactory.createLineBorder(Color.black), C.getName().toString()));
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (evt.getSource() instanceof JComboBox) {
                ...
                if (bx.getItemCount() < 1)
                    return;
                SetPrevOp();
                String opn = ((JComboBox) (evt.getSource())).getSelectedItem().toString();
    ...
    class FilePanel extends JPanel  implements ActionListener {
        ...
        private boolean check() {
            ...
            W.methPanel.valueChanged(new ListSelectionEvent(W.methPanel.Arguments, 0,
                    W.methPanel.Arguments.getComponentCount(), false));
        ...
        private String MultiLine_ify(String text, String prepend, 
            ...
            for (; j1 < text.length();) {
                Res += prepend + text.substring(j1, j2).trim() + append;
                j1 = j2 + 1;
                if (j1 + 1 < text.length())
                    j2 = text.indexOf("\n", j1);
                if( j2 <0)
    ...
    class MethInfData implements Serializable {
        ...
        public MethInfData(int argnum, String varName, String Prompt, 
                                                            String GUIParm) {

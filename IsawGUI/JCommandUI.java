...
/**
 * The class that displays various JTabbed panes containing different
 * information. Presently contains logtree, detector info, System info and a
 * command pane to run macro scripts.
 */
...
//      JScrollPane sp = new JScrollPane( table );
      ...
      textArea = new JTextArea( "Build Date: " + SharedData.BUILD_DATE + "\n" +
      ...
      "Total JVM Memory in bytes = " + rt.totalMemory() + "\n"
               ...
               + "\n" + "Java Vendor = "
               + SharedData.getProperty( "java.vendor" ) + "\n"
               + "Java Class Version = "
               + SharedData.getProperty( "java.class.version" ) + "\n"
               + "Java ClassPath = "
               + SharedData.getProperty( "java.class.path" ) + "\n"
               ...
               + "\n" + "\n" + "User Name = "
               + SharedData.getProperty( "user.name" ) + "\n"
               + "Current Directory = " + SharedData.getProperty( "user.dir" )
               + "\n" + "\n" + "File Separator= "
               + SharedData.getProperty( "file.separator" ) + "\n"
               + "Path Separator= " + SharedData.getProperty( "path.separator" )
               + "\n" + "\n" + "Operating System Name= "
               + SharedData.getProperty( "os.name" ) + "\n"
               + "Operating System Architecture= "
               + SharedData.getProperty( "os.arch" ) + "\n"
               + "Operating System Version= "
               + SharedData.getProperty( "os.version" ) );

      JScrollPane ta = new JScrollPane( textArea );

      // String tab_names [] = {"DataSet Log", "Session Log", "Detector Info",
      // "System Properties", "CommandPane"};

      jtp = new JTabbedPane();
      jtp.addTab( "Attributes" , pp );
      jtp.addTab( "DataSet Log" , pane );
      jtp.addTab( "Session Log" , new JPanelwithToolBar( "Save" ,
               new MySaveHandler() , njsp ) );
   ...
   public void setTab( String tab_title , JPanel obj ) {
      ...
      jtp.insertTab( tab_title , null , new JPanelwithToolBar( "EXIT" ,
               new MyHandler( obj ) , obj ) , null , 0 );
   ...
   public void showLog( DataSet ds ) {
      ...
      if( ds == null )
         return;
      if( ds.getTag() == current_dsTag ) // Only draw the log for a different
                                          // dataset.
         return;
      ...
      if( root.getChildCount() > 0 ) {
         DefaultMutableTreeNode dsnode = (DefaultMutableTreeNode) root
                  .getChildAt( 0 );
   ...
   public JTable showDetectorInfo( DataSet ds ) {
      ...
      try {
         ...
         for( int i = 0 ; i < ds.getNum_entries() ; i++ ) {
            ...
            detParamList[ i ][ 0 ] = new Integer( ( (Integer) ( attr_list
                     .getAttributeValue( Attribute.GROUP_ID ) ) ).intValue() );
            detParamList[ i ][ 1 ] = new Float( ( (Float) ( attr_list
                     .getAttributeValue( Attribute.RAW_ANGLE ) ) ).floatValue() );
            detParamList[ i ][ 2 ] = new Float( ( (Float) ( attr_list
                     .getAttributeValue( Attribute.INITIAL_PATH ) ) )
                     .floatValue() );
         }

         String[] columnHeading = {
                  "ID" , "Raw Angle" , "Flight Path" ,// "Start:Time(ms)",
                                                      // "End:Time(ms)"
                                                     // , "Number of Channels", "Total Count"
         };

         DefaultTableModel DTM = new DefaultTableModel( detParamList ,
                  columnHeading );
         table.setModel( DTM );

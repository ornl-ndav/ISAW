 ...
 *
 * Revision 1.4  2007/08/25 04:12:27  dennis
 * Removed redundant cast.
 *
 * Revision 1.3  2006/11/26 01:40:26  dennis
 * Changed to allow a null color.  If color is null, the last color
 * that was set will be used.
 * Added color as a fourth parameter to the getInstance method.  The
 * color can be null.
 *
 * Revision 1.2  2006/09/27 03:28:25  dennis
 * Fixed name on window of basic test method.
 *
 * Revision 1.1  2006/09/27 03:21:45  dennis
 * Copied from gov.pns.anl.ViewTools.Panels.GL_ThreeD.Shapes
 * package and updated it to use JSR231 (javax.media.opengl).
 * Removed calls to glEnable(GL_NORMALIZE) since such state
 * setting should be done in separate property setting nodes.
 *
...
public class Axis extends SimpleShape
{
  /**
  ...
  private Vector3D   p1[]     = null;                // end points p1 & p2 of
   ...
   *                         the numeric labels.
   *  @param  new_color      The color to use for the axis and text.
   */
                                  ...
                                  Color     new_color   )
  {
    ...
    if ( max_index == 0 )
    {
      ...
      axis = new Axis( start_point, end_point, 
                       min_val, max_val,
                       tick_dir, 
                       Axis.TOP, 
                       axis_label, 
  ...
  {
    super.setColor( new_color );
/*
    if ( axis_label != null )
      axis_label.setColor( new_color );

    if ( tick_labels != null )
      for ( int i = 0; i < tick_labels.length; i++ )
        tick_labels[i].setColor( new_color );
*/
  }
   ...
   *  Draw the axis, if the Axis was added to a scene graph.
   */
    ...
    if ( tick_labels != null )
      for ( int i = 0; i < tick_labels.length; i++ )
      {
        if ( tick_labels[i] != null )
          tick_labels[i].Render( drawable ); 
                                           // NOTE: Draw() method can only have
      }                                    //       methods callable inside of
                                           //       glNewList() and glEndList(),
                                           //       so we must call Draw() here,
                                           //       NOT Render()
    if ( axis_label != null )
      axis_label.Render( drawable );
  }
  ...
  { 
    ...
    for ( int i = 0; i < div_points.length; i++ )
    {
      ...
      StrokeText text = new StrokeText( ""+div_points[i], font, getColor() );
    ...
    { 
      ...
      axis_label = new StrokeText( label, font, getColor() );
  ...
  {
    float params[] = MathUtil.subDivide( min, max );
    float step = params[0];
  ...
  /* --------------------------- main ----------------------------------- */
  /**
   *  Main program that constructs an instance of several axes and displays
   *  them in 3D for testing purposes.
   */
  public static void main( String args[] )
  {
    ...
    Axis x_axis = getInstance( xmin, xmax, "X-Axis", Color.RED );    
    Axis y_axis = getInstance( ymin, ymax, "Y-Axis", Color.GREEN );    
    Axis z_axis = getInstance( zmin, zmax, "Z-Axis", Color.BLUE );    
...
//  demo.enableHeadlight( true );
    ...
    camera.setCOP( new Vector3D( 10, 10, 18 ) );
    camera.SetViewVolume( 1, 60, 40 );

...
#
QE_ds = ToQE( scat_fn_ds_ext_1, 0.0, 8.0, 250, Emin, Emax, 250, "QE.txt" )
SetField( QE_ds, "Title", "S(Q,E)" )
send QE_ds
QE_outfile_name = out_dir & "/QE_"&first_ds_index&"-"&last_ds_index&".isd"
Save QE_ds, QE_outfile_name
...
# results from other subsets of detectors.
#
G_outfile_name = out_dir & "/GFun_"&first_ds_index&"-"&last_ds_index&".isd"
Save Gsum_1_ds, G_outfile_name

#
# When testing this separately, we can load the G-Function we wrote to 
# verify that it was written correctly.
#
# Display "Loading " & G_outfile_name
# Load G_outfile_name, "loaded_ds"
# setInstrumentType( loaded_ds[0], "UNKNOWN" )
# send loaded_ds[0]

return "Wrote files: " & S_outfile_name & " and " & G_outfile_name

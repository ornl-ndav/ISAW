...
class TOPAZ_SNAP_spectrum(GenericTOF_SCD):

    def setDefaultParameters(self):
        self.super__clearParametersVector()
        self.addParameter(DataDirPG("Raw data path:", "/SNS/TOPAZ/2011_2_12_SCI/data/"))
        self.addParameter(StringPG("Run number of data file:", "4078"))
        self.addParameter(StringPG("Run number of background file:", "4087"))
        self.addParameter(StringPG("File name prefix (TOPAZ_ or SNAP_):", "TOPAZ_"))
        self.addParameter(StringPG("File name suffix (_histo.nxs or .nxs):", "_histo.nxs"))
        self.addParameter(IntegerPG("Number of border channels to exclude:", 16))
        self.addParameter(LoadFilePG("DetCal file:", "/SNS/users/ajschultz/TOPAZ_spectrum/TOPAZ_Detectors.DetCal"))
    ...
    def getResult(self):
        ...
        filename_prefix = self.getParameter(3).value
        filename_suffix = self.getParameter(4).value
        nBorder = self.getParameter(5).value
        DetCalFilename = self.getParameter(6).value
        doSmoothing = self.getParameter(7).value
        pointsLeft = self.getParameter(8).value
        pointsRight = self.getParameter(9).value
        polyDegree = self.getParameter(10).value
        V_rod = self.getParameter(11).value
    ...
    def __init__(self):
        Operator.__init__(self,"TOPAZ_SNAP_spectrum")

...
/**
 * This class feeds input to the cylabstof or fltabstof executable and parses its output using
 * Runtime.exec();
 */
public class CoralAbsTof {
  
  private static String coralexepath = CoralMulTof.coralexepath;
  public static String cylabstof = null,
                       fltabstof = null;
  
//  private boolean isCylinder;
  private String abstof;
  float[] angles;
  int nw, nan;
  float[] lambdas;
  float[][][] abscoeffs;
  ...
  static{
/*
    coralexepath = SharedData.getProperty( "ISAW_HOME" );
    if (coralexepath==null){
    coralexepath = SharedData.getProperty("user.home");
    coralexepath += java.io.File.separator+"ISAW";
    }
    coralexepath = coralexepath.trim();
    coralexepath += java.io.File.separator+"Operators"+java.io.File.separator+"TOF_Diffractometer";
*/
    String os = SharedData.getProperty("os.name");
    if (os.equals("Linux")) {
      cylabstof = coralexepath+java.io.File.separator+"cylabstof_x";
      fltabstof = coralexepath+java.io.File.separator+"fltabstof_x";
    } 
    else if (os.startsWith("Windows")) {
      cylabstof = coralexepath+java.io.File.separator+"cylabstof_w";
      fltabstof = coralexepath+java.io.File.separator+"fltabstof_w";
    } 
  ...
  CoralAbsTof (GLADScatter sca) {
    nan = sca.nan;
    angles = sca.angles;
    nw = sca.nlambda;
    absinput = new StringReader(sca.absinput);
    imask = sca.anmask;
//    isCylinder = !sca.isFlatPlate;
    if (sca.isFlatPlate) abstof = fltabstof;
    else abstof = cylabstof;
  }
  
  CoralAbsTof (HIPPOScatter sca) {
      nan = sca.nan;
      angles = sca.angles;
      nw = sca.nlambda;
      absinput = new StringReader(sca.absinput);
  ...
  private void exeAbsCorrection() throws IOException, InterruptedException{
    ...
    Process proc = (Runtime.getRuntime()).exec(abstof);
...
//    System.out.println("******can******\n");
    ...
    CoralAbsTof smprunabs = new CoralAbsTof(smprun);

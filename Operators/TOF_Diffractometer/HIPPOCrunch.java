 ...
 * 
 * $Log$
 * Revision 1.3  2006/07/27 15:11:15  dennis
 ...
 */
public class HIPPOCrunch implements Wrappable, IWrappableWithCategoryList { 
  //~ Instance fields **********************************************************
  
  private boolean DEBUG = false;
  public DataSet ds0;
  public LoadFileString runfile = new LoadFileString();
  public int numberofpulses = 1000;
  public boolean noDeadDetList = false;
  public LoadFileString redpar = new LoadFileString();
  public float lcutoff = HIPPORunProps.getfloatKey(HIPPORunProps.defHIPPOProps, "HIPPO.ANALYSIS.LCUTOFF");
...
//  public boolean DetSmoothing = false;
  ...
  public String getCommand(  ) {
    return "HIPPO_CRUNCH";                      //operator name used in ISAW scripts;
  }
...
//detector efficiency calculation, a porting of geffcyl.for code based on the method by J.M. Carpenter (IPNS note 17);
  public static float[] geffcyl (HIPPORunProps run, float[] lambdas, float psi) {
 
    float facd = ((Float) run.ExpConfiguration.get("HIPPO.DET.FACD")).floatValue();    
...
//normalization and rebin;  
  public static void hippo_norm (HIPPORunProps runinfo, DataSet ds, DataSet dm){
...
//    float min_d = 1.9f, max_d = 2.2f, min_x, max_x;
    ...
    float QMAX = ((Float)runinfo.ExpConfiguration.get("HIPPO.ANALYSIS.QMAX")).floatValue();
//    int NDETCHANNEL = ((Integer)runinfo.ExpConfiguration.get("GLAD.DET.NCHANNEL")).intValue();
//    float DETCHANNELWIDTH = ((Float)runinfo.ExpConfiguration.get("GLAD.DET.CHANNELWIDTH")).floatValue();
    float aream = ((float[])runinfo.ExpConfiguration.get("HIPPO.MON.AREA"))[0];
    float effm = ((float[])runinfo.ExpConfiguration.get("HIPPO.MON.EFF"))[0];
    float d1 = ((Float)runinfo.ExpConfiguration.get("HIPPO.BEAM.D1")).floatValue();
    System.out.println("moderator to sample distance: "+d1);
//    float d0 = ((Float)runinfo.ExpConfiguration.get("HIPPO.BEAM.D0")).floatValue();
...
/*                        
    ...
    tof_length_m = -(mon_position.getDistance())+ d1;
    System.out.println("tof_length_m: "+tof_length_m+" t_vals_m.length: "+t_vals_m.length);        
...
*/
    float npulses = ((Float)ds.getAttributeValue(Attribute.NUMBER_OF_PULSES)).floatValue();
    System.out.println("number of pulses: "+npulses);
    ...
    for (int i = 0; i < ds.getNum_entries(); i++){
      ...
      tof_length_d = d1 + data_params[1];
      psi = data_params[2];
      domega_d = data_params[3];
//      System.out.println(scattering_angle+" "+tof_length_d+" "+domega_d+" "+psi);
...
//      System.out.println("y_vals_dc[100]: "+y_vals_dc[100]);
...
//      System.out.println("W_vals_d[100]: "+W_vals_d[100]+" Q_vals_d[100]: "+Q_vals_d[100]);
...
//      System.out.println("y_vals_n[100]: "+y_vals_n[100]);
...
*/
        y_vals_n[k] = (float) (cc*cd/npulses);
      }
...
//    System.out.println("lambdamin: "+lambdamin+" lambdamax: "+lambdamax);
  ...
  public Object calculate(  ) {
    ...
    NexusRetriever nr = new NexusRetriever(runfile.toString());
    DataSet ds = nr.getDataSet(5); //10 degree bank;
    DataSet dm = nr.getDataSet(4);
    DataSet ds_10 = ds;
    expds.add(ds);
    ds = nr.getDataSet(13); //20 degree bank;
    expds.add(ds);
    ds = nr.getDataSet(16); //40 degree bank;
    expds.add(ds);
    ds = nr.getDataSet(7); //90 degree bank;
    expds.add(ds);
    ds = nr.getDataSet(10); //140+160 degree banks;
    expds.add(ds);
    System.out.println("\n"+ds.getAttributeValue(Attribute.RUN_TITLE)+"\n");
/*
    RunfileRetriever rr = new RunfileRetriever(runfile.toString());
    DataSet ds = rr.getDataSet(1);
    DataSet dm = rr.getDataSet(0);
...
//    System.out.println("correctDeadTime() takes "+time+" ms.");
    ...
    float d1, scattering_angle, domega, d2, psi, tof_length_d;
    float[] t_vals_d, y_vals_d;
    float[][] detparms;
...
//    d1 = ((Float)runinfo.ExpConfiguration.get("HIPPO.BEAM.D1")).floatValue();
    HIPPORunProps.Load_SED_Info(HIPPORunProps.HIPPODetTable);
    for (int index = 0; index < expds.size(); index++)
    {   
      detparms = HIPPORunProps.detParamVals[index];
      ds = (DataSet)expds.get(index);
      for (int i = 0; i < ds.getNum_entries(); i++){
        dt = ds.getData_entry(i);
        gid = dt.getGroup_ID();
        dt.setGroup_ID(gid + HIPPORunProps.bids[index]*1000);        
    
        scattering_angle = detparms[i][0]*3.1416f/180.0f;      
        d2 = detparms[i][1];
        psi = detparms[i][2]*3.1416f/180.0f;
        domega = detparms[i][3];     
        dt_grp_params = new Float1DAttribute (HIPPORunProps.HIPPO_PARM, new float[] {scattering_angle, d2, domega, psi});
...
*/
        if (index != 0 ) ds_10.addData_entry(dt);  
      }
      
    }
    
    ds = ds_10;
    ds.setAttribute(new FloatAttribute (Attribute.NUMBER_OF_PULSES, numberofpulses));
    gid = ds.getNum_entries();
    
    if (noDeadDetList) 
          runinfo.setBadDataGroups(ds, redpar.toString(), lcutoff);
    int nbadgrp;
    StringBuffer removedDataIDList = new StringBuffer();
    //remove dead detector data groups;
    nbadgrp = runinfo.removedDataID.length;
    for (int i = 0; i < nbadgrp; i++){
      removedDataIDList.append(runinfo.removedDataID[i] + " ");
      ds.removeData_entry_with_id(runinfo.removedDataID[i]);        
    ...
    System.out.println("Remove data with ID:\n"+removedDataIDList);    
    System.out.println("Use "+(gid-nbadgrp)+" data groups from this runfile.\n");    
    
    hippo_norm (runinfo, ds, dm);
...
//    ViewManager view_nrm = new ViewManager(ds, IViewManager.IMAGE);
/*

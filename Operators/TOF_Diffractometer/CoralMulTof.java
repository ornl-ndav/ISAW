 ...
 *
 * $Log$
 * Revision 1.2  2006/07/27 15:11:14  dennis
...
/**
 * This class feeds input to the cylmultof or fltmultof executable and parses its output using
 * Runtime.exec();
 */
public class CoralMulTof {

  public static String coralexepath;
  public static String cylmultof = null,
                       fltmultof = null;

  private String multof;
  float[] angles;
  int nw;
  float[] lambdas;
  float[][][] mulcoeffs;
  ...
  static{
    if ( (coralexepath = SharedData.getProperty( "ISAW_HOME" )) == null)
    {
    ...
    if (os.equals("Linux")) {
      cylmultof = coralexepath+java.io.File.separator+"cylmultof_x";
      fltmultof = coralexepath+java.io.File.separator+"fltmultof_x";
    } 
    else if (os.startsWith("Windows")) {
      cylmultof = coralexepath+java.io.File.separator+"cylmultof_w";
      fltmultof = coralexepath+java.io.File.separator+"fltmultof_w";
    } 
  ...
  CoralMulTof (GLADScatter sca) {
    angles = sca.angles;
    nw = sca.nlambda;
    mulinput = new StringReader(sca.mulinput);
    imask = sca.anmask;
    if (sca.isFlatPlate) multof = fltmultof;
    else multof = cylmultof;
  }
  
  CoralMulTof (HIPPOScatter sca) {
      angles = sca.angles;
      nw = sca.nlambda;
      mulinput = new StringReader(sca.mulinput);
      imask = sca.anmask;
  ...
  private void exeMulCorrection() throws IOException, InterruptedException{
    ...
    Process proc = (Runtime.getRuntime()).exec(multof);
...
//    System.out.println("******can******\n");
    ...
    CoralMulTof smprunmul = new CoralMulTof(smprun);

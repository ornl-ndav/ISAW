 ...
 *
 * $Log$
 * Revision 1.5  2006/04/27 22:41:36  taoj
 * Changes in the class static block, so it doesn't point to the static block of GLADRunProps class (which can autogenerate some output).
 *
...
public class MutCross {
  public static final float RTLAMBDA = 1.7979f;
  //the reference wavelength at 2200 m/s neutron
  ...
  private void getSigmaTable(String sigmatable)
    ...
    while ((line = fr_input.readLine()) != null) {
      if (line.charAt(0) != '#') {
        ...
        scattering_params =
          new float[] {
            (new Float(list[1])).floatValue(),
            (new Float(list[3])).floatValue(),
            (new Float(list[4])).floatValue(),
            (new Float(list[5])).floatValue()};
  ...
  */
  ...
  public float[] getTargetSigmas(String[] elements, float[] formula) {
    ...
    if (elements.length != formula.length) {
      ...
      sigma[1] /= sum_formula;
      bbarsq /= sum_formula;
      sigma[2] = bbarsq * bbarsq / 100.0f;
      //sigma[2] is (sum(f[i]*b[i],i=1..n)/sum(f[i],i=1..n))^2 in bars;
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
      "Default crosssection data file is " + MutCross.defSigmaTable + "\n");
    /*
       try {
         run();
       } catch(Throwable t) {

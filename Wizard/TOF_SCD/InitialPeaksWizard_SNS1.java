 ...
 */
public class InitialPeaksWizard_SNS1 extends Wizard {

  public InitialPeaksWizard_SNS1(  ) {
     this( true );
   }
    ...
    */
   public InitialPeaksWizard_SNS1( boolean standalone ) {
   ...
   //             new int[]{0,3}));
     ...
     //Xlate for OrientMat as form 2
     int[][] Xlate= { {28, 0, 0,-1, 0, 0}, //peaks vector
              {-1, 15, 1, 0,-1,-1}, //init UB matrix
              {23,12,-1,-1,-1,-1},
              {24,11,-1,-1,-1,-1},
              {-1,-1,-1, 5, 5,-1}, //Transformation{-1,-1,-1, 6, 5,-1}
              {-1,-1,-1,-1,12, 1},  //lsqrs UB matrix
              {1 , 1 ,6, -1,10, 6},//path{1 , 1 ,6, 3,10, 6}
              {4 ,-1 ,7,-1, -1,7}//,expname
     ...
     String helpURL = System.getProperty("ISAW_HOME");
     helpURL = helpURL.replace('\\','/');
     if( !helpURL.endsWith("/"))
        helpURL +="/";
     helpURL = "IPW_SNS1.html";
     //helpURL = "file://"+helpURL;
  ...
  public static void main( String[] args ) {
    InitialPeaksWizard_SNS1 w = new InitialPeaksWizard_SNS1( true );
    w.wizardLoader( args );
  }
 ...
 public static JMenu CreateMenuItems( String MenuText ){
    if( MenuText == null)
     ...
     Res.add( daily );
     initial.addActionListener( new InitialPeaksWizard_SNS1.WizardMenuListener());
     daily.addActionListener( new InitialPeaksWizard_SNS1.WizardMenuListener());

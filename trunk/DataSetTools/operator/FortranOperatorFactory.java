 ...
 * cannot be instantiated.  There is one extremely important rule:  If you want the 
 * Operators to show up in the menu correctly, your ISAW installation directory
 * should be similar to the stock install: i.e a directory starting with "ISAW" 
 * and the requisite directories of classes immediately below these.  
 * Like the JavaWrapperOperators,
 * these Fortran code sets must follow these rules:<br><br>
 * <ul>
 * <li> Only one function is allowed.  It must be called calculate, and it
 * may not have any arguments.</li>
 * <li> For functions that need arguments, global variables should be used.
 * </li>
 * </ul>
...
no single implicit variable initialization (e.g. creating an implicit integer and
initializing it like: imyint = 1 )...this is not a compiler, and I do not keep track of 
variables.  If I allow implicit variable initialization, then there is no way to do 
assignments.  It is allowed if there is more than one variable 
(e.g. myvar1 = 1, myvar2 = 2).

no implicit type conversion yet

no multidimensional character arrays

--------------------
IF statements
--------------------
single line if statements must truly be on a single line, like:
...
'else' statements are their own line, however:
...
if creating a multiple line if, there can be no single line if statements.  For example
this is invalid:
  if( x .ge. 5 ) x = 5 + 7
  elseif( tr .ge. 7 ) d = 9
  else
...
All labels for loops must be numbers
The step, if given, for a loop must be an actual number, not a variable
 */
public class FortranOperatorFactory {

  /**
   * DO NOT make this constructor public.  This is a factory, and should
   * not be instantiated.
   */
  ...
  /**
   * Returns a JavaWrapperOperator that was created by converting
   * Fortran code to a Wrappable class.  If there are errors in parsing
   * or reading the file, this will return null.
   * 
  ...
  /**
   * Creates a Wrappable Object from the Fortran code in the given file.
   * If there are errors in parsing or reading, this will return null.
   * 
  ...
  private static Wrappable createFortranWrapped( String fileName ) {
    ...
    try {
      ...
      compiler.setProperty( "userDirPath", SharedData.getProperty( "ISAW_HOME" ) );
      compiler.setProperty( "tempDirNameRoot", "" );
      myWrapped = 
        ( Wrappable ) ( compiler.compileClass( code ).newInstance(  ) );
    } catch( Exception iae ) {
   ...
   * underneath it without an extra layers (similar to the stock distribution).
   * 
   * @param file The File that contains the absolute fully qualified path of the
   * file on the filesystem.
   * 
   * @return A String representing the File's parent path, delimited by periods,
   * without the ISAW and previous part of the File's path.
   */
  public static String createPackageName( File file ) {
    ...
    folderPath = ( folderPath.substring( slashIndex + 1, folderPath.length(  ) ) ).replaceAll( "/", "." );
    return folderPath;
  }

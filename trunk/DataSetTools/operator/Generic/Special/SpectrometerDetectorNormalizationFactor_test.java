/*
 * File:  SpectrometerDetectorNormalizationFactor_test.java   
 *
 ...
 *
 *  $Log$
 *  Revision 1.8  2006/07/10 21:28:25  dennis
 ...
 *
 *  Revision 1.4  2004/03/15 06:10:50  dennis
 *  Removed unused import statements.
 *
 *  Revision 1.3  2004/03/15 03:28:35  dennis
 ...
 *  1/solid_angle.
 *  Minor changes include better error checking on the Attributes and
 *  a different arbitrary scale factor (1000000). Since the solid angles are
 *  very small the previous arbitrary scale factor (1000) still gave huge
 *  values for the normalization factors.
 *
...
public class SpectrometerDetectorNormalizationFactor_test
                                                     extends    GenericSpecial 
                                                     implements Serializable
{
  ...
  public SpectrometerDetectorNormalizationFactor_test( DataSet ds, float theta )
  {
    this();
  ...
  */
  ...
  public Object getResult()
  {
    ...
    for ( int j = 0; j < num_data; j++ )
    {
      ...
      if ( position == null )
        return new ErrorString("ERROR: DETECTOR_POS attribute needed in " +
                               "SpectrometerDetectorNormalizationFactor_test" );

      energy_in_obj=(Float)
                      attr_list.getAttributeValue(Attribute.ENERGY_IN);
      if ( energy_in_obj == null )
        return new ErrorString("ERROR: ENERGY_IN attribute needed in " +
                               "SpectrometerDetectorNormalizationFactor_test" );

      solid_angle_obj = (Float)
                         attr_list.getAttributeValue(Attribute.SOLID_ANGLE);
      if ( solid_angle_obj == null )
        return new ErrorString("ERROR: SOLID_ANGLE attribute needed in " +
                               "SpectrometerDetectorNormalizationFactor_test" );

      if( position != null && energy_in_obj != null && solid_angle_obj != null )
...
//                    (float)Math.PI, energy_in, THETS );
        ...
        */
        ...
        exp_FF[j] = sum / (solid_angle * 1000000);     
                                          // normalize to intensity/solid ang
...
//    ChopTools.chop_dataDrawer.drawgraphDataSet(new_ds);
  ...
  public Object clone()
  {
    SpectrometerDetectorNormalizationFactor_test new_op = 
                          new SpectrometerDetectorNormalizationFactor_test( );

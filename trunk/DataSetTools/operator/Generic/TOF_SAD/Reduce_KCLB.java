...
public class Reduce_KCLB extends GenericTOF_SAD {
   ...
   public Reduce_KCLB(DataSet TransS, DataSet TransB, DataSet Eff, DataSet Sens,
   ...
   public Object getResult() {
      ...
      if(RUNCds != null)
         RUNC = (((IntListAttribute) (RUNCds[0].getAttribute(Attribute.RUN_NUM))).getIntegerValue())[0]; else RUNC = -1;
      ITEMP[1] = LTEMP;
      sdt[1] = sdt[1];
      edt[1] = edt[1];
      tttt[1] = tttt[1];
      ...
      tofs = cnvertToWL(tofs, (XAxisConversionOp) RUNSds[1].getOperator("Convert to WaveLength"), NUMX * NUMY / 2);
      if(tofs[0] > tofs[1]) 
         Reverse(tofs);
   ...
   public Object init() {
      ...
      if(IF2D != 1) {
         ...
         Vector SSampQ = SumQs(SampGrid, xscl, RUNSds[0], SensGrid, Eff);
      ...
      LLOW = .5 * (LAMBDA[0] +  //Change to subrange of times
               LAMBDA[1]);
      PixelInfoList pilistx = ((PixelInfoList) RelSamp.getData_entry(0).getAttributeValue(Attribute.PIXEL_INFO_LIST));
      ...
      SQXQY = sss(DIVx, DIVy);
      SERRXY = sss(DIVx, DIVy);
      BQXQY = sss(DIVx, DIVy);
      BERRXY = sss(DIVx, DIVy);
      SBQXQY = sss(DIVx, DIVy); 
      ...
      for(int i = 0; i < RelSamp.getNum_entries(); i++) {
         ...
         IPixelInfo ipx = ((PixelInfoList) (RelSamp.getData_entry(i).getAttributeValue(Attribute.PIXEL_INFO_LIST))).pixel(0);
         ...
         Qxy = tof_calc.DiffractometerVecQ(detPos, L1, 1000f).getCartesianCoords();
   ...
   public static void main(String[] args) {
      ...
      Reduce_KCLB Reduce_KCL = new Reduce_KCLB(TransS[0], TransB[0], 
   ...
   //xscl is an XScale of Q values specified by the user 
   // Data block in SampGrid and SensGrid are have common llamda binning
   //   SumQs converts them to Q one data block at a time
   private Vector SumQs(UniformGrid SampGrid, XScale xscl, DataSet RunsDs, 
   ...
   private ErrorString CalcRatios(IDataGrid SampGr, IDataGrid CadmiumGr, 
      ...
      if(useTransmission)
         Transmy = Transm.getData_entry(0).getY_values(); else
         Transmy = null;
   ...
   private void ZeroSens(UniformGrid SensGrid, UniformGrid SampGrid) {
      ...
      for(int row = 1; row <= SensGrid.num_rows(); row++)
         for(int col = 1; col <= SensGrid.num_cols(); col++) {
            ...
            DetectorPosition dp = (DetectorPosition) (DD.getAttributeValue(
                     Attribute.DETECTOR_POS));

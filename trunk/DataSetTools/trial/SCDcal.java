                      ...
                      implements Serializable
{
   ...
   *  
   *  @param  instrument_type     One of PeakData_d.SNS_SCD, 
     ...
     for ( int i = 0; i < run.length; i++ )
     {
       ...
       { 
         ...
         gon_rotation.put( key, rotation );
                                         ...
                                         // initialize ordered array of all
                                         // detector grids
     grid_array = SCDcal_util.getAllGrids( grids );  

     nominal_position = new Vector3D_d[ grid_array.length ];
     nominal_base_vec = new Vector3D_d[ grid_array.length ];
     nominal_up_vec   = new Vector3D_d[ grid_array.length ];

     for ( int det_count = 0; det_count < grid_array.length; det_count++ )
     { 
       UniformGrid_d grid = grid_array[ det_count ]; 
       nominal_position[ det_count ] = grid.position();
       nominal_base_vec[ det_count ] = grid.x_vec();
...
*/
  ...
  protected double find_U_and_B_observed()
  {
    ...
    double my_qxyz[][] = SCDcal_util.copy( qxyz_observed );
  ...
  protected void find_qxyz_observed()
  {
    ...
    for ( int peak = 0; peak < n_peaks; peak++ )
    {
      ...
      SCDcal_util.mult_vector( rotation, sample_shift_array, coords );
      ...
      SCDcal_util.mult_vector( rotation_inv, coords, qxyz_observed[peak] );
    }
  }
  ...
  protected void find_qxyz_theoretical()
  {
    ...
    for ( int peak = 0; peak < n_peaks; peak++ )
      SCDcal_util.mult_vector( UB, hkl[peak], qxyz_theoretical[peak] );
  }
  ...
  {                           
    ...
    for ( int i = 0; i < all_parameters.length; i ++ )
    {
      if ( (i - DET_BASE_INDEX) % N_DET_PARAMS == 0 )
      {
        out.println();
        out.println();
        out.println("Parameters for Detector " + grid_array[det_count].ID());
        det_count++;
      }
      if ( used_p_index[i] >= 0 )
  ...
  public void ShowOldCalibrationInfo( PrintStream out )
  {
    ...
    out.println();
    out.println("===========================================================");
    out.println("BASIC Calibrated Values (IPNS SCD conventions)" );

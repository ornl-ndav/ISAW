...
import Operators.Special.DataSetArrayMerge_calc;
...
/**
 * This class provides a view of the data mapped onto the detector positions in
 * 3D. The Data Blocks in the DataSet must have "Detector Position" and/or
 * PixelInfoList attributes for this viewer to work.
 * 
...
public class ThreeD1View extends DataSetViewer
{
   ...
   private Hashtable< Integer , IDataGrid >              GridInfo              = new Hashtable< Integer , IDataGrid >( );

   private Hashtable< Integer , ImageFilled3DRectangle > GridImages            = new Hashtable< Integer , ImageFilled3DRectangle >( );

   private Hashtable< Integer , int[][] >                DSIndices             = new Hashtable< Integer , int[][] >( );
   ...
   public ThreeD1View(DataSet data_set, ViewerState state)
   {
      ...
      color_table = IndexColorMaker.getDualColorTable( getState( ).get_String(
            ViewerState.COLOR_SCALE ) , NUM_POSITIVE_COLORS );
      ...
      gov.anl.ipns.Util.Sys.PrintComponentActionListener.setUpMenuItem( jmb ,
            this );
      init( );
      initIndexPtrs( );
    ...
    * 
    * @param reason
    *           The reason the redraw is needed.
    * 
    */
   public void redraw(String reason)
   {
      ...
      if ( reason.equals( IObserver.SELECTION_CHANGED ) ) // no selection
                                                          // display yet
         return;
      ...
      else if ( reason.equals( IObserver.POINTED_AT_CHANGED ) )
      {
         ...
         if ( index != DataSet.INVALID_INDEX
               && !Float.isNaN( last_pointed_at_x ) )
         {
   ...
   /* ------------------------- getDisplayComponent -------------------------- */
   /**
    * Get the JComponent that contains the 3D view of the data, without any
    * associated controls or auxillary displays.
    */
    ...
    * conversions.
    * 
    * @return The current values from the number of bins control and the x range
    *         control form the Xscale that is returned, if no XScale is defined,
    *         a default XScale is used from a Data block in the DataSet.
    * 
    */
   ...
   private void AddOptionsToMenu()
   {
      ...
      JRadioButtonMenuItem r_button = new JRadioButtonMenuItem(
            GROUP_MARKER_SMALL );
   ...
   /**
    * Stores the data set indices for a given row and col of a grid
    */
   private void initIndexPtrs()
   {

      DataSet ds = getDataSet( );
      IDataGrid lastGrid = null;
      int[][] lastGridData = null;
      ...
      for( int i = 0 ; i < ds.getNum_entries( ) ; i++ )
      {
         ...
         if ( plist != null )
         {
            ...
            IDataGrid grid = pinf.DataGrid( );

            if ( grid != lastGrid )
            {
               if ( lastGrid != null )
                  DSIndices.put( lastGrid.ID( ) , lastGridData );

               lastGrid = grid;
               lastGridData = DSIndices.get( grid.ID( ) );

               if ( lastGridData == null )
               {
                  lastGridData = new int[ grid.num_rows( ) ][ grid.num_cols( ) ];

                  for( int r = 0 ; r < lastGrid.num_rows( ) ; r++ )
                     Arrays.fill( lastGridData[r] , -1 );
               }
            }
            int row = ( int ) ( .5 + plist.row( ) );
            int col = ( int ) ( .5 + plist.col( ) );
            lastGridData[row - 1][col - 1] = i;
         }
   ...
   /* ---------------------------- group_location --------------------------- */
   // Will NOT do Groups
   private Vector3D group_location1(int index)
   {
      ...
      Position3D position = ( Position3D ) d
            .getAttributeValue( Attribute.DETECTOR_POS );
   ...
   /* ------------------------------ set_colors ---------------------------- */
   /**
    * Set the color of all of the 3D group and/or detector objects to the color
    * corresponding to the data at the specified x_val.
    * 
    * @param x_val
    *           The x_value at which the data is to be evaluated.
    * 
    */
...
//         if ( !detector_draw_mode.equalsIgnoreCase( DETECTOR_NONE ) )
 //           threeD_panel.setColors( DETECTOR_PREFIX + i , color_table[index]);
   ...
   private void initFrameValues()
   {
      ...
      if ( ds.getData_entry( 0 ).isHistogram( ) )
         for( int i = 0 ; i < num_frames ; i++ )
            frame_vals[i] = ( x_vals[i] + x_vals[i + 1] ) / 2; // bin center for
                                                               // hist
      else
         for( int i = 0 ; i < num_frames ; i++ )
   ...
   /* -------------------------- findMaxAbsDataValue ----------------------- */
   /**
    * Find the overall maximum absolute value of the data values in the DataSet,
    * and record it in the field max_abs.
    */
   ...
   /* -------------------------- setDataScaleFactor ----------------------- */
   /**
    * Find the overall min and max data values in the DataSet, and set the scale
    * factor used to map the data values to the interval from
    * -(LOG_TABLE_SIZE-1) to +(LOG_TABLE_SIZE-1).
    */
   ...
   /* ------------------------------ remove_groups -------------------------- */
   /**
    * Remove the 3D objects representing average detector group positions from
    * the 3D display.
    */
   ...
   private float draw_detectors()
   {
      ...
      float max_radius = 0;
      int maxRowsCols = 0;
      IndexColorModel colorModel = color_model;
      ...
      for( int i = 0 ; i < grids.length ; i++ )
      {
         ...
         {
            int ID = grid.ID( );
            GridInfo.put( ID , grid );
            GridVirtArray2D GrInf = new GridVirtArray2D( grid , 0 , colorModel );

            mult = Math.min( mult , grid.width( ) );
            mult = Math.min( mult , grid.height( ) );
            ...
            if ( grid.num_rows( ) > maxRowsCols )
               maxRowsCols = grid.num_rows( );

            if ( grid.num_cols( ) > maxRowsCols )
               maxRowsCols = grid.num_cols( );

            ImageFilled3DRectangle R = new ImageFilled3DRectangle( grid
                  .position( ) , grid.x_vec( ) , grid.y_vec( ) , grid.width( ) ,
                  grid.height( ) , grid.num_rows( ) , grid.num_cols( ) , GrInf
                        .getVirtArray( ) , colorModel , this );
            ...
            IThreeD_Object[] obj = new IThreeD_Object[ 1 ];
            obj[0] = R;

            R.setPickID( DETECTOR_PREFIX + ID );

            threeD_panel.setObjects( DETECTOR_PREFIX + ID , obj );
         }

      }

      if ( mult < 2 * maxRowsCols )// entities should have at least one
      // pixel per row
      {
         mult = 2 * maxRowsCols / mult;
         ...
         for( int i = 0 ; i < grids.length ; i++ )
         {
            ...
            ImageFilled3DRectangle R = GridImages.get( grid.ID( ) );

            R.setScale( mult );
            R.setMaxMin( Max , Min );
            if( log_scale != null)
               R.setColorInfo( null , log_scale , -1);
   ...
   // deprecated for this application
   private float draw_detectors1()
   {
      ...
      {
         Data d = ds.getData_entry( i );
         PixelInfoList pil = ( PixelInfoList ) d
         ...
         if ( pil != null )
         {
            ...
            for( int k = 0 ; k < n_pixels ; k++ )
            {
               ...
               else
               {
                  ...
                  objects[k] = make_detector( point , base , up , pil.pixel( k )
                        .width( ) , pil.pixel( k ).height( ) , i );
               }
            }
   ...
   /**
    * Check for any null vector, or vector with a NaN component in the list.
    * 
    * @param list
    *           Array of vectors to check
    * @return true if any vector is null or has a NaN entry, and returns false
    *         if they are all OK.
    */
   ...
   /* ------------------------------ remove_detectors -------------------------- */
   /**
    * Remove the 3D objects representing detector pixel positions from the 3D
    * display.
    */
   ...
   private void draw_beam(float radius)
   {
      ...
      { // for depth sorting
         ...
         points[1] = new Vector3D( -i * radius / 3 , 0 , 0 ); // -x-axis
                                                              // direction
    ...
    */
   private IThreeD_Object make_detector(Vector3D point, Vector3D base,
         Vector3D up, float width, float length, int pick_id)
   {
      ...
      } else
      {
         ...
         gov.anl.ipns.ViewTools.Panels.ThreeD.Polygon object = new gov.anl.ipns.ViewTools.Panels.ThreeD.Polygon(
               verts , Color.red );
         if ( detector_draw_mode.equalsIgnoreCase( DETECTOR_HOLLOW ) )
            object
                  .setType( gov.anl.ipns.ViewTools.Panels.ThreeD.Polygon.HOLLOW );
   ...
   public static void main(String[] args)
   {
      ...
      String file_name = "C:/ISAW/SampleRuns/SNS/SNAP/SNAP_240.nxs";

      NexusRetriever ret = new NexusRetriever( file_name );
      DataSet data_set = null;

      for( int i = 1 ; i < ret.numDataSets( ) ; i++ )
      {
         ...
         if ( data_set == null )
            ...
            for( int d = 0 ; d < ds.getNum_entries( ) ; d++ )
               data_set.addData_entry( ds.getData_entry( d ) );

      }

      ThreeD1View view = new ThreeD1View( data_set , null );
   ...
   private void init()
   {
      ...
      TitledBorder border = new TitledBorder( LineBorder
            .createBlackLineBorder( ) , "Brightness" );
   ...
   private void processMouseClickEvent(MouseEvent e)
   {
      ...
      int DetID = index - DETECTOR_PREFIX;

      IDataGrid grid = GridInfo.get( DetID );
      ...
      DataSet ds = getDataSet( );
      Data D = grid.getData_entry( rc[0]+1 , rc[1]+1 );
      ...
      float y_val = D.getY_values( )[0];
      float x_val = D.getX_scale( ).getStart_x( );
      conv_table.showConversions( x_val , y_val , Index );
   ...
   /**
    * Listen for mouse motion events and just print out the pixel coordinates to
    * demonstrate how to handle such events.
    */
    ...
    * when it changes.
    */
   private class LogScaleEventHandler implements ChangeListener, Serializable
   {
   ...
   private class FrameControlListener implements ActionListener
   {
      ...
      public void actionPerformed(ActionEvent e)
      {
         ...
         if ( !Float.isNaN( frame_val ) && notify_ds_observers )
         {
            ...
            if ( debug )
               System.out
                     .println( "ThreeD viewer CALLING NOTIFY: " + frame_val );
   ...
   private class GroupDrawMenuHandler implements ActionListener, Serializable
   {
      ...
      public void actionPerformed(ActionEvent e)
      {
         ...
         {// Will not be working with groups
         ...
         Serializable
   {
      ...
      public void actionPerformed(ActionEvent e)
      {
         ...
         if ( action != detector_draw_mode )
         {
            ...
            getState( ).set_String( ViewerState.V_DETECTORS ,
                  detector_draw_mode );
   ...
   private class XScaleListener implements ActionListener, Serializable
   {
   ...
   public static Vector CalcData(IDataGrid grid, int channel)
   {

      float Max= Float.NEGATIVE_INFINITY;
      float Min =Float.POSITIVE_INFINITY;
      float[][] Res = new float[ grid.num_rows( ) ][ grid.num_cols( ) ];
      int N = grid.num_rows( );
      ...
      for( int row = 0 ; row < grid.num_rows( ) ; row++ )
         ...
         {
            int r1 = N-1-row;
            Data D = grid.getData_entry( r1 + 1 , col + 1 );
            
            Res[row][col] = D.getY_values( )[channel];
            if( Res[row][col] < Min)
               Min = Res[row][col];
            if( Res[row][col] > Max)
               Max = Res[row][col];
   ...
   /**
    * Class that handles creating Virtual Array 2D from a Data Grid
    * 
    * @author ruth
    * 
    */
   class GridVirtArray2D
   {
      ...
      public GridVirtArray2D(IDataGrid grid, int channel, IndexColorModel model)
      {
         Vector R = CalcData( grid , channel );
         Min = ((Float) R.elementAt( 2 )).floatValue( );
         Max = ((Float) R.elementAt( 1 )).floatValue( );

         V2D = new VirtualArray2D( (float[][])R.firstElement() );
         this.grid = grid;
         this.model = model;
      }

      /**
       * Updates the value of the VirtualArray2D from the given channel of the
       * data set
       * 
       * @param channel
       *           The channel(=0 for this application)
       */
      public void UpdataData(int channel)
      {
         Vector R = CalcData( grid , channel );
         Max =((Float) R.elementAt( 1 )).floatValue( );
         Min =((Float) R.elementAt( 2 )).floatValue( );
      ...
      public IVirtualArray2D getVirtArray()
      {
      ...
      public void setColorModel(IndexColorModel model)
      {
   ...
   private class AltAzControlListener implements ActionListener, Serializable
   {

/*
 * File:  SegInfoListAttribute.java
 *
 ...
 *
 *  $Log$
 *  Revision 1.1  2003/01/15 20:12:14  dennis
 *  Renamed from 'Detector' to 'Segment'
 *
...
/**
 * The concrete class for an attribute whose value is a list of SegmentInfo
 * objects.  
 *
...
public class SegInfoListAttribute extends Attribute implements  IXmlIO
                                                   
{
  ...
  private SegmentInfo[] values;

  /**
   * Constructs a SegInfoListAttribute object using the specified name 
   * and values.
   */
  public SegInfoListAttribute( String name, SegmentInfo value[] )
  {
    super( name );
    ...
    values = new SegmentInfo[ length ];

    for ( int i = 0; i < values.length; i++ )
      values[i] = new SegmentInfo( value[i] );
    ...
    if ( !in_order )
    {
      System.out.println("Sorting SegmentInfoListAttribute");
      Arrays.sort( values, new SegmentInfoComparator() );
    }
  }


  private SegInfoListAttribute()
  {
    super("");
    values = new SegmentInfo[0];
 
  }

  /**
   * Returns reference to the array of SegmentInfo objects for this attribute,
   * as a generic object. 
   */
   ...
   * new value is the result of merging the two lists of SegmentInfo objects.
   *
  ...
  {
    if ( !(attr instanceof SegInfoListAttribute) )      // can't do it so don't 
      return this;                                          
       
    SegInfoListAttribute other_attr = (SegInfoListAttribute)attr; 

//    if ( values.length+other_attr.values.length > 20 )// ineffiecient for
//      return;                                         // long lists, just keep
                                                        // up to 20.

    SegmentInfo temp[] = new SegmentInfo[
    ...
    else
    {
      ...
      num_used += other_attr.values.length - j;
                                                  ...
                                                  // attribute
    SegmentInfo new_values[] = new SegmentInfo[ num_used ];
    System.arraycopy( temp, 0, new_values, 0, num_used );
    
    return new SegInfoListAttribute( name, new_values );
  }
  ...
  /**
   * Returns a string representation of the list of SegmentInfo objects 
   */
  ...
  * SegmentInfoListAttribute. In standalone mode it writes the xml header.
  *
  ...
  */
  ...
  {try
     ...
     if( values.length <1 )
       return true;
     SS.append( "<SegInfoListAttribute>\n");
     SS.append( "<name>");
     SS.append(name);
     SS.append("</name>\n");
     SS.append( "<value>"+"\n");
     SS.append( "<SegmentInfoList size =\"");
      SS.append(""+values.length+"\">\n");
     stream.write(SS.substring(0).getBytes());
     for( int i=0; i<values.length ; i++)
        values[i].XMLwrite( stream, mode);
     stream.write("</SegmentInfoList>\n".getBytes());
     stream.write( ("</value>"+"\n").getBytes());
     stream.write("</SegInfoListAttribute>\n".getBytes());
     return true;
     }
    catch( Exception s)
  ...
  * SegInfoListAttribute. In standalone mode it writes the xml header.
  *
  * @param  stream  the OutputStream to which the xml data is to be written
  * @param  mode    Either IXmlIO.Base64 to write spectra information 
  *                 efficiently or IXmlIO.Normal to produce ASCII values
   ...
   try
     {  
        ...
        if( !xml_utils.skipAttributes( stream ))
          return xml_utils.setError(xml_utils.getErrorMessage());
        if(!Tag.equals("name"))
           return xml_utils.setError(" No Name tag in SegInfoList");
...
//---------------------------------------
        ...
        if( Tag == null)
          return xml_utils.setError(xml_utils.getErrorMessage());
        if(!Tag.equals("SegmentInfoList"))
           return xml_utils.setError(
                             " Need SegmentInfoList tag in SegInfoList");
...
//--------------------------------------
        ...
        for( int i = 0; i< size; i++)
         { 
           ...
           if( Tag == null)
             return xml_utils.setError(xml_utils.getErrorMessage());
           String cl_name ="";
           if(!Tag.equals("SegmentInfo"))
             return xml_utils.setError( "Tag must be Struct in Seg Info");
           
           if( !xml_utils.skipAttributes( stream ))
              return xml_utils.setError(xml_utils.getErrorMessage());
           values[i] = new SegmentInfo();
        ...
        if( !xml_utils.skipAttributes( stream ))
          return xml_utils.setError(xml_utils.getErrorMessage());
        if( !Tag.equals("/SegmentInfoList"))
          return xml_utils.setError("Improper end Tag in SegInfoList");
        ...
        if( !Tag.equals("/value"))
          return xml_utils.setError(
                           "Improper SegInfoList Attr end Tag in SegInfoList");
        ...
        if( !xml_utils.skipAttributes( stream ))
          return xml_utils.setError(xml_utils.getErrorMessage());
        if( !Tag.equals("/SegInfoListAttribute"))
          return xml_utils.setError("Improper value end Tag in SegInfoList"+Tag+
            "/SegInfoListAttribute");
 ...
 */
                                                        ...
                                                        ClassNotFoundException
  {
    ...
    if ( IsawSerialVersion != 1 )
      System.out.println("Warning:SegInfoListAttribute IsawSerialVersion != 1");
  }

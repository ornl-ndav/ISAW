...
# run files:
$VanFile	LoadFileString("C:\Isaw\SampleRuns\HIPPO_E000002_R012066.nx.hdf") Vanadium run file:
$CanFile        LoadFileString("C:\Isaw\SampleRuns\HIPPO_E000002_R012067.nx.hdf") Sample run file:
$SmpFile	LoadFileString("C:\Isaw\SampleRuns\HIPPO_E000002_R012082.nx.hdf")  Can run file:
$BkgFile        LoadFileString("C:\Isaw\SampleRuns\HIPPO_E000002_R012077.nx.hdf") Background run file:

# number of pulses:
$npvan Float(38425332) Number of pulses: vanadium
$npcan Float(30050284)
$npsmp Float(30095450)
$npbkg Float(37576968)

# dead detector list:
$REDPAR LoadFileString("C:\Isaw\hipporun.par") Bad detector list file:
...
runinfo=HIPPO_CONFIGURE(vanopt, smpcomposition, smprho, canopt, scc)

mon_nrm_van=HIPPO_CRUNCH(runinfo, VanFile, npvan, true, REDPAR)
mon_nrm_can=HIPPO_CRUNCH(runinfo, CanFile, npcan)
mon_nrm_smp=HIPPO_CRUNCH(runinfo, SmpFile, npsmp)
mon_nrm_bkg=HIPPO_CRUNCH(runinfo, BkgFile, npbkg)
...
smo_van=HIPPO_VANCAL(runinfo, nrm_van, doSmooth, VanTemp)
Div(nrm_can, smo_van, false)
Div(nrm_smp, smo_van, false)

nrm_can=HIPPO_ANALYZE(runinfo, nrm_can, 2)
Sub(nrm_smp, nrm_can, false)
nrm_smp=HIPPO_ANALYZE(runinfo, nrm_smp, 3)

HIPPO_DISTINCT(runinfo, nrm_smp, nrm_van, SmpTemp, Wmin, Wmax)

if DoMerge	
	nrm_smp=HIPPO_COMBINE(nrm_smp, nrm_van)
endif
send nrm_smp

...
$ Ein_estimate      Float(90.5)                             Estimated Incident Energy
$ specify_t0_shift  BooleanEnable(false,1,6)                 Specify or Calculate t0_shift
...
$ last_ds_index     Integer(75)                            Largest DataSet Index ( num det banks )
$ banks_per_process Integer(3)                              Number of detector banks for each process
$ mem_per_process   Integer(2000)                           Megabytes per process
$ queue             String("mikkcomp")                      SLURM queue name
$ max_simultaneous_processes  Integer(36)                   Max number of cores to use
...
for i in [0:num_processes]
  ...
  if ( index_1 <= last_ds_index )
    ...
    QE_File_name[i]  = out_dir & "/QE_"   & index_1 & "-" & index_2 &".isd"
...
#   Display "Loaded " & SFunFile_name[i]
  ...
  On Error
    Load QE_File_name[i], "QE_loaded_ds"
    setInstrumentType( QE_loaded_ds[0], "UNKNOWN" )
    QE_list[QE_count] = QE_loaded_ds[0]
    QE_count = QE_count+1
#   Display "Loaded " & SFunFile_name[i]
  End Error
...
#
# Merge and save the S(Q,E) partial results 
#
Combined_QE = MergeQE( QE_list, "/home/dennis/QE_out.txt" )
send Combined_QE

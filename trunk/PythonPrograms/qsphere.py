...
# radius of sphere in units of 2pi/d
...
sigI = 0.0
rflg = 0
peakct = 0.0   # counts within radiusQ
bkgct = 0.0    # counts between radiusQ and 2*radiusQ
...
for i in range(numberOfEvents):
	...
	# Test for event within 2 times the sphere radius
    Qpeak = [peaks[pki][16][0], peaks[pki][16][1], peaks[pki][16][2]]
    Qdist = distance(Qpeak, qxyz)
    if Qdist > (2.0 * radiusQ): continue
    
    if Qdist < radiusQ:
        peaks[pki][17] = peaks[pki][17] + 1   # peak counts
    else:
        peaks[pki][18] = peaks[pki][18] + 1   # background counts
...
# peakVol = (4.0/3.0) * math.pi * radiusQ**3
# backgdVol = 7.0 * peakVol   # 8*peakVol - peakVol
ratio = 1.0 / 7.0
...
for i in range(dc.nod):
    ...
    for j in range(numOfPeaks):
        if peaks[j][15] == dc.detNum[i]:
            ...
            peakct = peaks[j][17]
            bkgct = peaks[j][18]
            intI = peakct - (ratio * bkgct)
            sigI = math.sqrt(peakct + (ratio**2 * bkgct))

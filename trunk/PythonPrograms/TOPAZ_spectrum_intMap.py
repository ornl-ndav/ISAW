...
#  Creates a map file of the intensity variation across each detector.
#  August 2011
...
class TOPAZ_spectrum(GenericTOF_SCD):
    ...
    def setDefaultParameters(self):
        ...
        self.addParameter(DataDirPG("Raw data path:", "/SNS/users/ajschultz/spectrum/TOPAZ_3023_3163"))
        self.addParameter(StringPG("Run number of data file:", "3023"))
        self.addParameter(StringPG("Run number of background file:", "3163"))
        ...
        self.addParameter(DataDirPG("Directory for output spectrum and log files:", "/SNS/users/ajschultz/spectrum/TOPAZ_3023_3163"))
    ...
    def getResult(self):
        ...
        # Open detector intensity map file
        filename = outPath + 'Det_intensity_' + runNum_1 + '.dat'
        mapFile = open( filename, 'w' )
        
        #  Obtain scaling factor from beam monitor data
        filename = path + 'TOPAZ_' + runNum_1 + '_histo.nxs'
        ...
        filename = path + 'TOPAZ_' + runNum_2 + '_histo.nxs'
...
#!        for i in range(12, number_of_detectors):
            ...
            mapFile.write( 'Detector Bank = %d     DetNum = %d\n' % (bank, DetNum[i]) )
            filename = path + 'TOPAZ_' + runNum_1 + '_histo.nxs'
...
#---------------------------------------------------------------
            ...
            for ix in range(32):
                ...
                for iy in range(32):
                    ...
                    #  Sum up the spectra from the region
                    sum_ds_1 = SumCurrentlySelected(ds_1, 1, 1).getResult()
                    sum_ds_2 = SumCurrentlySelected(ds_2, 1, 1).getResult()
                    ...
                    #  Sum all TOF channles
                    id = sum_ds_1.getData_entry(0).getGroup_ID()
                    sum_V = IntegrateGroup( sum_ds_1, id, 0.0, 17000. ).getResult()

                    id = sum_ds_2.getData_entry(0).getGroup_ID()
                    sum_bkg = IntegrateGroup( sum_ds_2, id, 0.0, 17000. ).getResult()

                    sum_V_bkg = sum_V - (sum_bkg*scale)  # subtract background

                    x_pos = (first_col + last_col + 1)/2
                    y_pos = (first_row + last_row + 1)/2
                    mapFile.write( ' %5d %5d %12.1f\n' \
                        % ( x_pos, y_pos, sum_V_bkg ) )
#---------------------------------------------------------------
    ...
    def __init__(self):
        Operator.__init__(self,"TOPAZ_spectrum_intMap")

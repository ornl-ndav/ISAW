 ...
 *
 * $Log$
 * Revision 1.2  2005/12/15 20:52:55  dennis
...
/**
 * This class feeds input to the Fltabstof executable and parses its output using
 * Runtime.exec();
 */
public class FltAbsTof {
  
  public static String fltabstof = null;
  ...
  static{
    fltabstof = SharedData.getProperty( "ISAW_HOME" );
    if (fltabstof==null){
    fltabstof = SharedData.getProperty("user.home");
    fltabstof += java.io.File.separator+"ISAW";
    }
    fltabstof = fltabstof.trim();
    fltabstof += java.io.File.separator+"Operators"+java.io.File.separator+"TOF_Diffractometer";
    String os = SharedData.getProperty("os.name");
    if (os.equals("Linux")) fltabstof += java.io.File.separator+"fltabstof_x";
    else if (os.startsWith("Windows")) fltabstof += java.io.File.separator+"fltabstof_w";
    if( new File(fltabstof).exists()){
        // do nothing
    } else{
      fltabstof = null;
    }
  ...
  private void exeAbsCorrection() throws IOException, InterruptedException{
    ...
    Process proc = (Runtime.getRuntime()).exec(fltabstof);
    ...
    if ( (nlambda=(new Integer(s)).intValue()) != nw ) 
       System.out.println("***UNEXPECTED ERROR***\ncheck number of lambdas in fltabstof output.\n");
...
//               System.out.println("wavelength_list "+"["+i+"]"+"["+j+"]: "+wavelength_list[j]+A);
        ...
        System.out.println("running fltabstof executable..."); 
...
//    System.out.println("******can******\n");
    ...
    FltAbsTof smprunabs = new FltAbsTof(smprun);

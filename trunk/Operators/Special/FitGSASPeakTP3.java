...
/**
 *  This class fits a GSAS TOF profile function 3 to a peak plus linear 
 ...
 *  FitGaussianPeak.java is used as the template. 
 */
public class FitGSASPeakTP3 implements Wrappable, IWrappableWithCategoryList
{
  ...
  */
  ...
  public String getDocumentation() 
  {
    ...
    s.append( "@overview This operator fits a GSAS TOF profile function 3 to a single " );
    ...
    s.append( "initial estimate of the Gaussian variance. Lorentzian " );
    s.append( "FWHM and linear background parameter starting values are zero.");
    s.append( "A Marquardt type algorithm is used to " );
    s.append( "optimize the parameters for a GSAS TOF profile function 3 function " );
    ...
    s.append( " 'gamma'\n" );
    s.append( "  gamma_value\n" );
    s.append( "  error_in_gamma\n" );
    s.append( " 'm'\n" );
    s.append( "  m_value\n" );
    s.append( "  error_in_m\n" );
    s.append( " 'y0'\n" );
  ...
  {
    if ( data_set == null )
      return new ErrorString("Null DataSet in FitGSASPeakTP3");

    if ( data_set == DataSet.EMPTY_DATA_SET )
      return new ErrorString("Empty DataSet in FitGSASPeakTP3");

    int n_data = data_set.getNum_entries();
    if ( n_data <= 0 )
      return new ErrorString("No Data blocks in DataSet in FitGSASPeakTP3"); 
    Data d = data_set.getData_entry_with_id( group_id );
    if ( d == null )
      return new ErrorString("ID " + group_id + 
                             " not in DataSet in FitGSASPeakTP3");

    XScale x_scale = d.getX_scale();
    if ( min_x < x_scale.getStart_x() )
      return new ErrorString("Interval extends below x_min in FitGSASPeakTP3");
  
    if ( max_x > x_scale.getEnd_x() )
      return new ErrorString("Interval extends above x_max in FitGSASPeakTP3");
...
*/
  ...
  public static void main( String args[] )
  {
    ...
    FitGSASPeakTP3 op_core = new FitGSASPeakTP3();
    op_core.data_set = ds;
    op_core.group_id = 23;
    op_core.min_x    = 20600;
    op_core.max_x    = 21400;

...
/**
  * This operator multiplies each y value in a Data block, by a corresponding
  * delta_x value.  For Data blocks that are histograms, the counts in 
  * the ith bin are multiplied by the bin width.  In particular,  yi 
  * is multiplied by (xi+1 - xi).  For Data blocks that are functions, all
  * but the last data value are multiplied by the NEXT interval length. That is,
  * for functions, yi is also multiplied by (xi+1 - xi), for all but the last
  * data point.  For functions, the last data point is multiplied by the 
  * PREVIOUS interval length, since there is not a next interval.   
  */

public class MultiplyByDeltaX extends    AnalyzeOp 
                              implements Serializable
{
  ...
  public MultiplyByDeltaX( )
  {
    super( "Multiply sample values yi, by delta_xi" );
  }
  ...
  public MultiplyByDeltaX( DataSet  ds,
                           boolean  make_new_ds )
  {
   ...
   */ 
   public String getDocumentation()
   {
     ...
     Res.append("@overview This operator multiplies each y value in a ");
     ...
     Res.append("the original DataSet values will be altered by multiplying ");
     ...
     Res.append("then multiplied by the bin width." );
     ...
     Res.append("All but the last y-value will then be multiplied by the ");
     Res.append("length of the NEXT interval.  The last y-value will be ");
     Res.append("multiplied by the length of the PREVIOUS interval.");
     ...
     Res.append("String that reads \"Data multiplied by delta_x\".");
  ...
  *  that says "Data multiplied by delta_x".
  */
  ...
  {                                // get the parameters specified by the user 
    ...
    new_ds.addLog_entry( "Multiplied by delta_x" );
    ...
    else
    {
      ...
      return new String( "Data multiplied by delta_x" );
    }
  }  
  ...
  /* ------------------------------ clone ------------------------------- */
  /**
   * Get a copy of the current MultiplyByDeltaX Operator.  The list 
   * of parameters and the reference to the DataSet to which it applies are 
   * also copied.
   */
  public Object clone()
  {
    DataSetOperator new_op = new MultiplyByDeltaX( );
  ...
  {
    MultiplyByDeltaX op = new MultiplyByDeltaX();

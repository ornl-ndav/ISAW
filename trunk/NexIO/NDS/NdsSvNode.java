...
public class NdsSvNode implements NxNode
  { public Vector Node;//1st filename,NXFile, 2nd node name comma class class
   ...
   public Vector getNode(){return Node;}
   public int getNChildNodes()
     {String S1;
      ...
      errormessage="";
      try{
      //System.out.println("getNchild Node2 args="+getFile()+","+getDef()+"NXVGROUP");
      ...
      catch(IOException s){ errormessage= s.getMessage();
                            return -1;}
      n=0;
 ...
 public NxNode getChildNode(String NodeName)
   {String S1;
      ...
      try{
      //System.out.println("get child args to ="+","+getFile()+","+getDef()+"NXVGROUP");
      if(!nds.getVGroupDirectory(getFile(), getDef()+"NXVGROUP"))
          {errormessage=nds.getStringData();
   ...
   public int getNAttributes()
     {errormessage="";
      ...
      if(!nds.getAttrib( getFile(),getDef()+"/NXVGROUP"))
       ...
       catch(IOException s){errormessage=s.getMessage(); return -1;}
   ...
   public Object getNodeValue()
    { errormessage="improper node for Value";
      ...
      if(k==NDSClient.FLOAT)
         ...
         {errormessage="imporper return type";return null;}
         ...
         int k=Aname.indexOf('@');
          ...
          if(Aname.length()>=k+3)if(Aname.indexOf("SDS",k+1)==k+1)
             {String key=Aname.substring(0,k);
              ...
              if(i<0)i=0;
              if(SS.charAt(i)=='/')SS=SS.substring(0,i+1);
              else SS="/";
              SS=SS+"SDS "+key;
              ...
              if(!nds.getData(getFile(),SS))
                ...
                else if(k==NDSClient.INT) X=(Object)nds.getIntData();
                else if(k==NDSClient.BYTE) X=(Object)nds.getByteData(); 
              else if(k==NDSClient.STRING) X=(Object)nds.getStringData();   
              else if(k==NDSClient.DOUBLE)X=(Object)nds.getDoubleData(); 
                 else //if(k==NDSClient.FILE) 
               {errormessage="imporper return type";return null;}

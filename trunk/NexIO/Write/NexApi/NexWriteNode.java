...
/** Class that handles writing information to a Nexus file
...
{ protected String errormessage;
  ...
  */
   ...
   *Note: the node_class should be a Nexus class like NXentry, NXdata,etc.
   */
...
//------------------ Links ----------------------------
   ...
   *@param linkhandle  the name used to refer to this linked information
   */
    ...
    *@param  handleName the name that will be used to refer to this linked 
    *                   information
    */
     ...
     if( classname.equals( "SDS" ) )
       {
        ...
        if( ranks ==  null ) 
         ...
         if( (value == null ) ||( ranks == null )  )
            {
             if( Debug)
               System.out.println( "END null Writing"+ nodename+","+
                       classname );
             return;
            }     
       
        nf.makedata( nodename , TypeConv.convertFrom( type ) , 
                          ranks.length , ranks );
  ...
  private int convertArrayR( int rankoffset , Object value , int valueoffset ,
     ...
     {int i;
      if( rankoffset == ndims - 1 )
        { System.arraycopy( value , valueoffset , Result , 0 , 
                                     ranks[ ndims-1 ] );
       ...
       { 
        try{
             if( rankoffset+1 < ndims-1 )
                R[ i ] = ( Object )Array.newInstance( C , 
                                     ranks[  rankoffset+1 ] );
             else 
                R[ i ] = ( new NxNodeUtils() ).CreateArray( type , 
                                         ranks[ ndims - 1 ] );
            }
         catch( Exception s )
         ...
         n = convertArrayR( rankoffset + 1 , value , n , type , ndims , 
                           ranks , R[ i ] );
    ...
    /**
 * Converts a linear array to a multidimesioned array.
 * Non-linear values are just returned.
*/
  private Object convertArray( Object value , int type1 , int ndims ,
                               int ranks[] )
    {
     errormessage = "";
     if( ndims <= 1 )
...
public static void main1( String args[] )
  {
   ...
   ranks[ 0 ] = 9;
   ndims = 1;
   nw.addAttribute( "filenamee" , ( Object )
                     ( new String( "Hi There" ).getBytes( ) ) ,
   ...
   while( c!= 'x' )
    ...
    { nnn.write();
     ...
     {Object Res =  ( nnn.convertArray (  values , type ,  ndims , ranks ) );
      ...
      System.out.println( Res.getClass()+"::"+( new NxNodeUtils() ).Showw(  Res ) );

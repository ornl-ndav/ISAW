...
public class NxWriteData{
  ...
  public boolean processDS( NxWriteNode nodeEntr ,  NxWriteNode nxInstr ,  
    ...
    for( j=0; j < DS.getNum_entries() ; j++){
      ...
      for(  i = 0 ; (i < DS.getNum_entries()) &&( !already) ; i++ ){
         ...
         if( i < j) 
           ...
           DetectorPosition dp =(DetectorPosition) DB.getAttributeValue(Attribute.DETECTOR_POS);
           if( dp != null){
              float[] coords = dp.getSphericalCoords();
              coords = Types.convertToNexus( coords[0],coords[2],coords[1]);
              distance = UnionF( distance, new Float( coords[0]));
              polar = UnionF( polar,  new Float( coords[1]));
              azim = UnionF( azim,  new Float( coords[2]));
           }
           id = UnionI( id, new Integer( DB.getGroup_ID()));
           solidAngle = UnionF(solidAngle, DB.getAttributeValue(Attribute.SOLID_ANGLE));
           rawAngle = UnionF(rawAngle, DB.getAttributeValue(Attribute.RAW_ANGLE));
           TotCount = UnionF(TotCount, DB.getAttributeValue(Attribute.TOTAL_COUNT));
           DeltToTheta = UnionF(DeltToTheta, DB.getAttributeValue(Attribute.DELTA_2THETA));
           slot = UnionI(slot, DB.getAttributeValue( Attribute.SLOT));
           crate = UnionI(crate, DB.getAttributeValue( Attribute.CRATE));
           input = UnionI(input, DB.getAttributeValue( Attribute.INPUT));
    ...
    if(!already){ //make an NXdata and Nxdetector
          ...
          NxWriteNode datanode=nodeEntr.newChildNode( DS.getTitle()+"_"+k, "NXdata");
          NxWriteNode detnode =nxInstr.newChildNode( DS.getTitle()+"_"+k, "NXdetector");
          NxWriteNode dnode = util.writeFA_SDS( datanode,"data",yvals, NexIO.Inst_Type.
                       makeRankArray(yvals.length/ny_s,ny_s,-1,-1,-1));
         
          util.writeStringAttr( dnode,"units", "Count");
          util.writeIntAttr(dnode,"signal", 1);
          util.writeStringAttr(dnode,"label", DS.getTitle());
          util.writeStringAttr(dnode,"link", DS.getTitle()+"_"+k);
          dnode = util.writeFA_SDS( datanode,"errors",errors,NexIO.Inst_Type.
                       makeRankArray(errors.length/ny_s, ny_s,-1,-1,-1));
         
          dnode = util.writeFA_SDS( detnode,"time_of_flight",DBM.getX_scale().getXs(),NexIO.Inst_Type.
          ...
          dnode = util.writeIA_SDS( detnode,"id",id, NexIO.Inst_Type.makeRankArray( id.length,
               -1,-1,-1,-1));
          dnode.setLinkHandle("id"+DS.getTitle()+"_"+k);
          datanode.addLink("id"+DS.getTitle()+"_"+k);
          util.writeIntAttr( dnode,"axis",2);
          dnode = util.writeFA_SDS( detnode, "distance",distance, NexIO.Inst_Type.makeRankArray(
             distance.length,-1,-1,-1,-1));
          util.writeStringAttr( dnode,"units","m");
          dnode = util.writeFA_SDS( detnode, "polar_angle",polar, NexIO.Inst_Type.makeRankArray(
             polar.length,-1,-1,-1,-1));
          util.writeStringAttr( dnode,"units","radians");
          dnode = util.writeFA_SDS( detnode, "azimuthal_angle",azim, NexIO.Inst_Type.makeRankArray(
             azim.length,-1,-1,-1,-1));
          util.writeStringAttr( dnode,"units","radians");

          dnode = util.writeFA_SDS( detnode, "polar",polar, NexIO.Inst_Type.makeRankArray(
             polar.length,-1,-1,-1,-1));
          util.writeStringAttr( dnode,"units","radians");


          dnode = util.writeFA_SDS( detnode, "solid_angle",solidAngle, NexIO.Inst_Type.makeRankArray(
             solidAngle.length,-1,-1,-1,-1));
          


          dnode = util.writeFA_SDS( detnode, "raw_angle",rawAngle, NexIO.Inst_Type.makeRankArray(
             rawAngle.length,-1,-1,-1,-1));
          util.writeStringAttr( dnode,"units","degrees");


          dnode = util.writeIA_SDS( detnode, "slot",slot, NexIO.Inst_Type.makeRankArray(
             slot.length,-1,-1,-1,-1));


          dnode = util.writeIA_SDS( detnode, "crate",crate, NexIO.Inst_Type.makeRankArray(
             crate.length,-1,-1,-1,-1));
         


          dnode = util.writeIA_SDS( detnode, "input",input, NexIO.Inst_Type.makeRankArray(
             input.length,-1,-1,-1,-1));
         

          dnode = util.writeFA_SDS( detnode, "total_count",TotCount, NexIO.Inst_Type.makeRankArray(
             TotCount.length,-1,-1,-1,-1));
          util.writeStringAttr( dnode,"units","count");
       
          int[] rank = NexIO.Inst_Type.makeRankArray(
             DeltToTheta.length,-1,-1,-1,-1);
          dnode = util.writeFA_SDS( detnode, "delta_to_theta",DeltToTheta, rank);
          util.writeStringAttr( dnode,"units","radians");
         k++;
  ...
  private float[] UnionF( float[] Array, Object newElement){
    ...
    if( !(newElement  instanceof Number)) return null;
   ...
   private int[] UnionI( int[] Array, Object newElement){
    ...
    if( !(newElement instanceof Integer))return null;
...
private float[] UnionF( float[] Array,float[] newElement){
    ...
    if( Array== null) return null;
    if( newElement== null) return null;  
    if(!(newElement instanceof float[]))return null;
   ...
   private int[] UnionI( int[] Array, int[] newElement){
    ...
    if( Array== null) return null;
    if( newElement== null) return null; 
    if(!(newElement instanceof int[]))return null; 
 ...
 private boolean ProcessGroupedGrids( NxWriteNode nodeEntr, NxWriteNode nxInstr, 
   ...
   for( int i = 0; i < NGridClasses; i++){
      ...
      float[][] orientation = new float[NDet][3];
      ...
      boolean is_slot=true, is_crate = true, is_input = true;
     
      Fill( row_cm); Fill(col_cm);
        ...
        if( GridClass[first] == GridClass[j]){
          ...
          for( int row =1; row <= grid.num_rows(); row++)
           for( int col=1; col <= grid.num_cols(); col++){
             ...
             System.arraycopy( db.getY_values(),0, data[det][row-1][col-1],0,ny_s);
             System.arraycopy( db.getErrors(),0, errors[det][row-1][col-1],0,ny_s);
          }
          id[det] = grids[j];
          DetNums[det] = grids[j];
          slot[det] = ConvertDataTypes.intValue( d.getAttributeValue(Attribute.SLOT));
          crate[det] = ConvertDataTypes.intValue( d.getAttributeValue(Attribute.CRATE));
          input[det] = ConvertDataTypes.intValue( d.getAttributeValue(Attribute.INPUT));
          if( slot[det] < 0 ) is_slot=false;
          if( crate[det] < 0 ) is_crate=false;
          if( input[det] < 0 ) is_input=false;
      ...
      //Create the NXdata
       if(!is_slot) slot = null;
       if(!is_crate) crate = null;
       if(!is_input) input = null;
       NxWriteNode nxData = nodeEntr.newChildNode( DS.getTitle()+"_G2"+det,"NXdata");
       NxWriteNode nxDetector = nxInstr.newChildNode( DS.getTitle()+"_G2"+det,"NXdetector");
       if(CreateG3NXdetectorNode( nxDetector,id,slot,crate,input,distance,azim,
               polar,orientation, width,height, DS, xvals,det))
          return true;
  ...
  private boolean CreateG3NXdetectorNode( NxWriteNode nxDetector,int[]id,
               ...
               DataSet DS,float[] tof,int det){
      Inst_Type inst = new Inst_Type();
      NxWriteNode node = util.writeIA_SDS( nxDetector,"id",id,inst.makeRankArray(
                id.length,-1,-1,-1,-1));
      if(slot!=null) node = util.writeIA_SDS( nxDetector,"slot",slot,inst.makeRankArray(
                slot.length,-1,-1,-1,-1));
      if(crate!=null)node = util.writeIA_SDS( nxDetector,"crate",crate,inst.makeRankArray(
                crate.length,-1,-1,-1,-1));
      if(input!=null)node = util.writeIA_SDS( nxDetector,"input",input,inst.makeRankArray(
                input.length,-1,-1,-1,-1));
      node = util.writeFA_SDS( nxDetector,"distance",distance,inst.makeRankArray(
                distance.length,-1,-1,-1,-1));
          util.writeStringAttr( node, "units", "m");
      node = util.writeFA_SDS( nxDetector,"azimuthal_angle",azim,inst.makeRankArray(
                azim.length,-1,-1,-1,-1));
         util.writeStringAttr( node,"units","radians");
      node = util.writeFA_SDS( nxDetector,"polar_angle",polar,inst.makeRankArray(
                polar.length,-1,-1,-1,-1));
         util.writeStringAttr( node,"units","radians");
      node = util.writeFA_SDS( nxDetector,"width",width,inst.makeRankArray(
 ...
 private boolean CreateG3NXdataNode( NxWriteNode nxData, float [][][][] data,
      ...
      node = util.writeFA_SDS( nxData,"x_offset",row_cm, util.setRankArray(row_cm));
      util.writeIntAttr( node, "axis", 2);
      util.writeStringAttr( node, "link",DS.getTitle()+"_G2"+det);
      util.writeStringAttr( node, "label",DS.getTitle());
 
      node = util.writeFA_SDS( nxData,"y_offset",col_cm, util.setRankArray(col_cm));
      util.writeIntAttr( node, "axis", 3);
     
      node = util.writeFA_SDS( nxData,"DetIDS",DetNums, util.setRankArray(DetNums));
  ...
  public boolean processDSgrid1( NxWriteNode nodeEntr, NxWriteNode nxInstr, 
     ...
     NxWriteNode Nxdata = nodeEntr.newChildNode( DS.getTitle()+"_G1"+GridNum,
                                                   "NXdata");
     NxWriteNode Nxdetector = nxInstr.newChildNode( DS.getTitle()+"_G1"+GridNum,
                                         "NXdetector");
     ...
     xoffset.addAttribute("link",(DS.getTitle()+"_G1"+GridNum+(char)0).getBytes(),
            NexIO.Types.Char, NexIO.Inst_Type.makeRankArray( (DS.getTitle()+"_G1"+GridNum).
               length()+1,-1,-1,-1,-1));
     float[]  col_cm;
    ...
    if( input != null){
      ...
      //NexIO.Inst_Type.makeRankArray(MIN_GRID_SIZE,-1,-1,-1,-1));
    }
    if( solidAngle != null){

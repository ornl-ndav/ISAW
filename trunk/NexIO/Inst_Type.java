...
public class Inst_Type
{
...

   {
      ...
      if( S.equals( "phi" ) )
      {
         ...
         nphi.addAttribute( "long_name", 
                ( "Scattering Angle" + ( char )0 ).getBytes(), Types.Char,
      ...
      DataSet DS, int beginIndex, int endIndex )
   {
      ...
      {
         NxWriteNode ntof = makeXvalnode( DS, beginIndex, endIndex,
                                          nxdetector );
      ...
      { //phi will be 2nd axis
         ...
         for( int i = beginIndex; i < endIndex; i++ )
         {
            ...
            phi[i - beginIndex] =
                  ( ( DetPosAttribute )A ).getDetectorPosition().
                                              getScatteringAngle();
   ...
   )
   {
      ...
      {
         ntof.addAttribute( "units", 
                      ( DS.getX_units() + ( char )0 ).getBytes(), Types.Char,
                       makeRankArray( DS.getX_units().length() + 1, 
                       -1, -1, -1, -1 ) );
      }

      ntof.addAttribute( "long_name", 
               ( DS.getX_label() + ( char )0 ).getBytes(), Types.Char,

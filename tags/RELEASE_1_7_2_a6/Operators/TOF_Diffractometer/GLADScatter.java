...
/**
 * This class provides the IPNS GLAD instrument and experimental specific information
...
//furnace:
    ...
    astep = getfloatKey(expsetup.ExpConfiguration, analykeyh+GLADRunProps.ASTEP);
    mstep = getfloatKey(expsetup.ExpConfiguration, analykeyh+GLADRunProps.MSTEP);
    minw = getfloatKey(expsetup.ExpConfiguration, analykeyh+".MUT.MINW");
    maxw = getfloatKey(expsetup.ExpConfiguration, analykeyh+".MUT.MAXW");
    dw = getfloatKey(expsetup.ExpConfiguration, analykeyh+".MUT.DW");
    ...
    density = getfloatKey(expsetup.ExpConfiguration, expkeyh+GLADRunProps.DENSITY);

    sigma_a = getfloatKey(expsetup.ExpConfiguration, expkeyh+GLADRunProps.SIGMA_A);    
    sigmavs = MutCross.loadSigmaTable().getTargetSigmas(symbol, formula);
    sigma_s = sigmavs[0];
    bbarsq = sigmavs[2];
    if (sigma_a == 0.0) sigma_a = sigmavs[1];
...
//note: clone() here is probably a shallow copy;
    ...
    combos.rad = comborad;
    combos.scatterern = (float)(density*bht*Math.PI*(comborad[1]*comborad[1]-comborad[0]*comborad[0]));
...
//      System.out.println(MutTable);
  ...
  private String getAbsMulInputHead () {
    ...
    StringBuffer input = new StringBuffer(n+"\n");
    for(int i=0; i<n; i++){
  ...
  public void setAbsInput () {
    absinput = getAbsMulInputHead() + astep +"\n"
              +getAbsMulInputBody()
              +muttable;    
  }
  
  public void setMulInput () {
    mulinput = getAbsMulInputHead() + mstep +"\n"
              +getAbsMulInputBody()
              +muttable;
  }

...
#================= Parameters ======================================
#configuration:
$Configfile LoadFileString("/IPNShome/taoj/cvs/ISAW/InstrumentInfo/IPNS/smptubecan.dat") Configuration file:
...
$datadir DataDirectoryString(/IPNShome/glad/data) IPNS run file directoryy:
$bkgruns Array([10091]) Background run files numbers:
$vanruns Array([10088,10089]) Vanadium run files numbers:
$smpruns Array([10100:10102]) Sample run files numbers:
$canruns Array([10068:10070]) Can run files numbers:
$furruns Array([10107]) Furnace run file numbers:

# dead detector list:
$REDPAR LoadFileString("/IPNShome/taoj/cvs/ISAW/InstrumentInfo/IPNS/gladrun10068.par") Bad detector list file:
...
# DISTINCT (subtract the self-scattering part from differential cross section, also prepare the flux function for weighting) setup:
$SmpTemp Float(193.0)	Sample temperature:
...
runinfo=GLAD_CONFIGURE(vanopt, smpcomposition, smprho, canopt, 0, configfile, true, true, false, false, 0)
...
index = 0
for i in furruns
  furfile = datadir&"glad"&i&".run"
  Display "furnace runfile: " & furfile
  mon_nrm_fur[index]=GLAD_CRUNCH(runinfo, furfile)
  index = index + 1
endfor
mon_nrm_fur_sum=GLAD_SUMRUN(mon_nrm_fur)
...
nrm_fur=mon_nrm_fur_sum[1]

Sub(nrm_van, nrm_bkg, false)
Sub(nrm_can, nrm_bkg, false)
Sub(nrm_smp, nrm_bkg, false)
Sub(nrm_fur, nrm_bkg, false)

smo_van=GLAD_VANCAL(runinfo, nrm_van, doSmooth, VanTemp)
Div(nrm_can, smo_van, false)
Div(nrm_smp, smo_van, false)
Div(nrm_fur, smo_van, false)

#nrm_fur=GLAD_ANALYZE(runinfo, nrm_fur, 4)
#Sub(nrm_smp, nrm_fur, false)
#nrm_can=GLAD_ANALYZE(runinfo, nrm_can, 6)
#Sub(nrm_smp, nrm_can, false)
nrm_smp=GLAD_ANALYZE(runinfo, nrm_smp, 7)
